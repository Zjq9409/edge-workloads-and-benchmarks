...
class CallVLM:
    ...
    def _call_vlm(self, prompt, image_url):
        ...
        payload = {
            ...
            "max_tokens": 128,
            "stream": True # 必须开启流式输出才能测 TTFT/TPOT
        }
        ...
        for line in response.iter_lines():
            if line:
                if ttft is None:
                    ...
                    tokens_time.append(time.perf_counter())
        end_time = time.perf_counter()

        # 计算 TPOT: 所有 token 生成间隔的平均值
        tpot = 0
        if len(tokens_time) > 1:
            tpot = (tokens_time[-1] - tokens_time[0]) / (len(tokens_time) - 1)
        ttft = ttft * 1000
        tpot = tpot * 1000
    ...
    def process_frame(self, frame: VideoFrame) -> bool:
        ...
        try:
            ...
            with frame.data() as data:
                ...
                self._call_vlm("what is this", "file:/llm/models/test/test.jpg")
